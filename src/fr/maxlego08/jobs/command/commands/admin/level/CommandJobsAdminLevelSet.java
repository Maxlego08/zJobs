...
public class CommandJobsAdminLevelSet extends VCommand {

    public CommandJobsAdminLevelSet(ZJobsPlugin plugin) {
        super(plugin);
        this.setPermission(Permission.ZJOBS_ADMIN_LEVEL_SET);
        this.addSubCommand("set");
        this.setDescription(Message.DESCRIPTION_ADMIN_LEVEL_SET);
    ...
    protected CommandType perform(ZJobsPlugin plugin) {
        ...
        plugin.getJobManager().updatePlayerJobAttribute(sender, player, jobName, level, AdminAction.SET, AttributeType.LEVEL);
