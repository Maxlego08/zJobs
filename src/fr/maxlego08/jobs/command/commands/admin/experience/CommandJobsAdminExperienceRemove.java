...
public class CommandJobsAdminExperienceRemove extends VCommand {

    public CommandJobsAdminExperienceRemove(ZJobsPlugin plugin) {
        super(plugin);
        this.setPermission(Permission.ZJOBS_ADMIN_LEVEL_REMOVE);
        this.addSubCommand("remove");
        this.setDescription(Message.DESCRIPTION_ADMIN_EXPERIENCE_REMOVE);
    ...
    protected CommandType perform(ZJobsPlugin plugin) {
        ...
        plugin.getJobManager().updatePlayerJobAttribute(sender, player, jobName, experience, AdminAction.REMOVE, AttributeType.EXPERIENCE);
